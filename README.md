...
| **DevOps / Platform Engineering** | Source control ↔ CI/CD ↔ compute ↔ storage ↔ workstations, wired together |
...
### 🧊 Zen Server
I want Zen to be **observable**, not an opaque black box. When an asset keeps re-triggering compilation, I want to trace: what Unreal believes → what Zen believes → which changelist/engine version caused it → is it machine-specific or studio-wide. I've explored exposing this diagnostic layer through **MCP tooling** so agents (human or AI) can query it too.

### 🌍 Multi-site infrastructure
Designed architecture across studios in **Berlin, Finland, and Bratislava** — Perforce, Horde, Zen, TeamCity, Jenkins, RoboMerge, GitHub, Gitea. My default pattern: **central control, local execution** — policy and orchestration stay central; latency-sensitive stuff (caches, agents, compute) lives close to developers.
...
- **Migrate incrementally.** `Jenkins → partial Horde adoption → broader rollout`, never a big-bang rewrite.
